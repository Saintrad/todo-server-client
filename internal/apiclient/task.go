...
func (c *Client) GetTask(id int) (Task, error) {
	var out Task
	_, err := c.do(http.MethodGet, "/v1/tasks/"+strconv.Itoa(id), nil, &out)
	return out, err
}

func (c *Client) UpdateTask(id int, req UpdateTaskRequest) (Task, error) {
	var out Task
	_, err := c.do(http.MethodPatch, "/v1/tasks/"+strconv.Itoa(id), req, &out)
	return out, err
}

func (c *Client) DeleteTask(id int) error {
	_, err := c.do(http.MethodDelete, "/v1/tasks/"+strconv.Itoa(id), nil, nil)
	return err
}
